...
sequenceDiagram
    ...
    HOOK->>FS: counts reports in .claude/audit-usage/ and tests rule inference
    HOOK->>FS: validates .mcp.json (if it exists)
    HOOK->>FS: docker compose ps + docker ps (if a compose file and docker exist)
...
> runs but `pricing.json` is empty, so the reports show no cost.
