...
- Path to `docs/use-cases/UC-NNN-<slug>/UC-NNN-spec.md` — complete, 5 sections + checklist,
  plus an optional § 6 (messaging) when the use case has external delivery
- Project generated by `/init-project` — `pom.xml`, `src/`, `.claude/`
- Validation: spec.md exists, `status: approved`, has § 1-5 (§ 6 optional), checklist
  19/19 (or aborts) — § 6's presence doesn't change the 19
...
- `.claude/rules/naming.md`, `.claude/rules/code-quality.md`, `.claude/rules/lombok.md` —
...
- `.claude/rules/testing.md` — Block 4
  ...
  This scope line only says what's in reach. It doesn't cause a `Read` by itself — each
  file above is cited again inline at the block that needs it, and that inline citation,
...
## Procedure — 4 fixed blocks + 1 conditional

Order: domain (1-7) → persistence (8-12) → REST (13-15) → messaging (M1-M4, only when the
spec carries § 6) → tests (16-19). **Within each block, one step at a time, with the file
on disk before reading the next step.** Block M doesn't renumber the fixed checklist — it
stays 19 items whether or not Block M runs; a spec without § 6 skips straight from Block 3
to Block 4, same as it always did.
...
Rules: `.claude/rules/naming.md`, `.claude/rules/code-quality.md`, `.claude/rules/lombok.md`,
`.claude/rules/value-objects.md` (when a field earns a VO vs. stays primitive),
`.claude/rules/error-handling.md` (exception taxonomy for invariant violations),
`.claude/rules/architecture-ddd.md` § Domain (zero framework, zero I/O, `Clock` injected,
past-tense events). `.claude/rules/logging.md` applies too, in the negative: this layer logs
nothing — that silence is the rule, not an oversight.
...
Rules: `.claude/rules/naming.md`, `.claude/rules/code-quality.md`, `.claude/rules/lombok.md`,
`.claude/rules/architecture-ddd.md` § Adapters (translate, no business logic), and
`.claude/rules/logging.md`: `[Resource]RepositoryAdapter` logs query identifier + duration
only past the slow-query threshold `persistence.md` sets — silent otherwise.
...
Rules: `.claude/rules/naming.md`, `.claude/rules/code-quality.md`, `.claude/rules/lombok.md`,
`.claude/rules/error-handling.md` (the exception-family → status mapping
`ApiExceptionHandler` implements), and `.claude/rules/logging.md`: `[Resource]Controller`
logs method + path + response status + duration — `INFO` on success, `WARN`/`ERROR` on
failure per `error-handling.md`'s mapping.
...
Next: § 6 present → Messaging (steps M1-M4). Absent → Tests (steps 16-19)
```

### Block M: Messaging (steps M1-M4, conditional)

**Runs only when the spec carries § 6** — i.e., `new-feature`'s messaging step wasn't
skipped and `25-mensageria.md` exists. Check with `grep -m1 '^## 6' <spec>`; no match,
...
- `[Event]Publisher.java` — implements the outbound port `domain-modeling` already
  declared (`Publish[Event]Port`); `KafkaTemplate`, partition key = aggregate id
- `[Event]Payload.java` (`adapter/in/messaging`) — mirrors the producer's shape
- `[Event]Listener.java` — `@KafkaListener`, dedupes via `ProcessedEventStore` before
  calling the target use case's inbound port, manual `Acknowledgment` committed only
  after that call returns
- `ProcessedEventStore` port + its persistence adapter — only if Block 2 modeled the
  dedupe table per `25-mensageria.md` step 5a. Missing table and the partial named the
  need → spec defect, stop and report; this block doesn't design schema
- `application-kafka.yml` additions — bootstrap servers, producer `acks`/idempotence,
  consumer group id, offset reset, `DefaultErrorHandler` (retry backoff + DLQ topic)

Rules: `.claude/rules/messaging.md` (topic naming, idempotent consumer, retry/DLQ —
a business rejection from the consumed use case skips retry and goes straight to the
DLQ), `.claude/rules/architecture-ddd.md` § Adapters (translate, no business logic),
`.claude/rules/naming.md`, `.claude/rules/code-quality.md`, `.claude/rules/lombok.md`,
`.claude/rules/error-handling.md` (which exception is a business rejection vs. a retry
candidate), and `.claude/rules/logging.md`: `[Event]Listener` logs event type +
...
- Consumer: OrderConfirmedListener — group inventory-service, manual ack
...
- **Messaging norm** (`@.claude/rules/messaging.md`) — topic naming, idempotent consumer,
  retry/DLQ, applied only in Block M
- **D32** — `messaging-architect` skill scope; Block M closes the gap D32/`new-feature`
  flagged as pending ("known gap: the executor's fixed 19-step checklist doesn't yet
  implement a messaging block")
