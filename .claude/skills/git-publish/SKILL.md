...
description: >
  ...
  feature implemented successfully, or whenever the user asks to create a git repo,
  commit, or push the current project.
...
- Invoked by `/new-feature` right after `java-spring-boot-developer` reports the
  executor finished, with the UC name/summary as context.
...
3. `git add -A` (respecting `.gitignore`, already generated by the Initializr).
4. `git commit -m "<message>"` — build the message from the input context following
   ...
   call, or a generic `chore: commit pending changes` with no context. Always append:
